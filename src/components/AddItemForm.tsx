...
export function AddItemForm({ onItemAdded, categories }: AddItemFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    condition: 'NM' as ItemCondition,
    packagingState: 'CIB' as PackagingState,
    value: '',
    purchasePrice: '',
    imageUrl: '',
    acquisitionDate: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addItem(newItem);
    notify(`"${newItem.name}" added to ${addToOption}`, 'success');
    onItemAdded();
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <label className="block text-sm font-medium text-dark-200 mb-2">Packaging</label>
          <select
            required
        ...
        {isAdvancedOpen && (
          <div className="p-4 bg-dark-800/50 border-t border-dark-700 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-dark-200 mb-2">Purchase Price</label>
                <input
                  type="number"
                  step="0.01"
